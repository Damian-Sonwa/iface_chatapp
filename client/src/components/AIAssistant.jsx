    ...
    <div className="rounded-2xl p-6 md:p-8 border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1">
        {messages.filter(m => m.role !== 'system').map((m, idx) => (
          <motion.div key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`max-w-[80%] px-4 py-2 rounded-xl ${m.role==='user' ? 'ml-auto bg-purple-500 text-white rounded-tr-sm' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-tl-sm'}`}> 
            {m.content}
          </motion.div>
        ))}
        {loading && (
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <motion.button onClick={send} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} disabled={loading || !input.trim()} className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-60 flex items-center gap-2">
